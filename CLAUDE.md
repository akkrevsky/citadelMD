...
| 3080 | dsh (web UI) | deepseek-harness agent UI. Internal only — no host port, never publish it (no auth of its own) |
| 8082 | dsh gate (web nginx) | Local-only vhost with `auth_request` → backend `/api/auth/me`; NOT forwarded by the remote nginx |
...
| `DEEPSEEK_API_KEY` | dsh | DeepSeek platform key for the agent LLM (native API; same sk- token as `ANTHROPIC_AUTH_TOKEN` usually works) |
| `DEEPSEEK_BASE_URL` | dsh | Optional LLM endpoint base override (empty = public API) |
| `DSH_MODEL` | dsh | Agent model id on the native API (default `deepseek-v4-pro`) |
| `DSH_MCP_TOKEN` | dsh | apiKey of the `harness` service user — generated by `make -C infra agent-key` |
...
- `:8082` (separate vhost) — AI agent UI → dsh:3080, gated by `auth_request` → `/api/auth/me`. dsh serves its own SPA that calls a root-absolute `/api` — that is why it cannot share the 8081 origin (conflicts with the backend `/api`) and lives on its own port. Local-only for now.

### AI agent (dsh / deepseek-harness)

- `dsh` Docker service runs the deepseek-harness web UI in an isolated container (pinned RC versions — the project is a developer preview with compatibility-breaking changes; bump deliberately, one version at a time).
- Home-level patch `$DSH_HOME/cordis.patch.yml` (seeded by `infra/dsh/entrypoint.sh` from `infra/dsh/profile/`) configures: `agent-default-model` → provider `deepseek-official` + `DSH_MODEL`; `llm-deepseek` → `apiKeyEnv: DEEPSEEK_API_KEY`; `webserver` → 0.0.0.0:3080; plus an inserted `@deepseek-ai/dsh-mcp-client` row pointing at `http://mcp-server:3100/mcp` with `Bearer $DSH_MCP_TOKEN`.
- The agent reaches citadelMD documents only through the MCP tools (`mcp__citadelmd__*`); its bash/files tools are sandboxed to its own container — the docs git repo is NOT mounted.
- The agent acts as the `harness` service user (ADMIN, apiKey). That key lives only in `infra/.env` (gitignored) and the dsh container env.
- Frontend: home page shows the agent in an iframe (`http://<hostname>:8082/`) with a «Агент / Чат» toggle; the old chat and `/api/ai/chat` stay intact.
