...
**Критерии готовности:**
- [ ] Двое одновременно пишут в один документ — курсоры видны, мердж корректный.
- [ ] Правки из web появляются в working tree через 5 сек (auto-save).
- [ ] `POST /commit` создаёт новую версию, включая самые свежие правки (через flush).
- [ ] `POST /discard` откатывает файл и все подключённые клиенты получают откат.
- [ ] Закрытие документа (последний disconnect) = финальный flush в файл.
