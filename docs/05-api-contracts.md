...
Дерево доступных папок с документами. Каждый пользователь получает
личную корневую папку `users/<login>` (создаётся лениво при первом
обращении); ADMIN видит все личные папки, остальные — свою плюс папки,
к которым им выдан явный доступ через `folder_permissions` (поднимаются
на верхний уровень, если их предки недоступны). Владение папкой даёт
подразумеваемый ADMIN над её поддеревом.
